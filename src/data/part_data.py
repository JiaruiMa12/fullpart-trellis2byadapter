...
class DataConfig_3DMaster_Part(InstantiateConfig):
    ...
    trellis2_part_slat_dir: Optional[str] = None
    trellis2_global_slat_dir: Optional[str] = None
    ...
    slat_stats_json: Optional[str] = None
    """Path to custom slat stats json (e.g., trellis2_global_shape_latents/stats.json)"""
    trellis2_slat_norm: bool = False
    trellis2_slat_stats_json: Optional[str] = None
...
class Data:
    def __init__(self, config: DataConfig_3DMaster_Part, timer=None):
        ...
        self.trellis2_part_slat_dir = config.trellis2_part_slat_dir
        self.trellis2_global_slat_dir = config.trellis2_global_slat_dir
        ...
        # Load custom stats json if provided (e.g., for trellis2 32-dim latents)
        if config.slat_stats_json is not None and os.path.exists(config.slat_stats_json):
            import json
            with open(config.slat_stats_json, 'r') as f:
                slat_normalization = json.load(f)
            print(f"===Loaded slat normalization from {config.slat_stats_json}===")
            print(f"   Mean dim: {len(slat_normalization['mean'])}, Std dim: {len(slat_normalization['std'])}")
        else:
            # Default 8-dim normalization for trellis1
            slat_normalization = {
                "mean": [
                    -2.1687545776367188,
        ...
        if config.trellis2_slat_stats_json is not None and os.path.exists(config.trellis2_slat_stats_json):
            with open(config.trellis2_slat_stats_json, 'r') as f:
    ...
    def process_items(self, items):
        ...
        samples_slat_feats = []
        samples_slat_coords = []
        samples_trellis2_slat_feats = []
        samples_trellis2_slat_coords = []
        # caption
        samples_caption = []
        # id
        samples_id = []
        ...
        for idx in range(len(item_part)):
            try:
                ...
                slat_coords_list = []
                slat_feats_list = []
                trellis2_slat_coords_list = []
                trellis2_slat_feats_list = []
                ...
                if self.global_part:
                    ...
                    if self.stage == '2':
                        ...
                        if self.trellis2_global_slat_dir is not None:
                            trellis2_global_slat_path = os.path.join(self.trellis2_global_slat_dir, self.id_to_sha256[part_id] + '.npz' if self.id_mapping else part_id + '.npz')
                            if os.path.exists(trellis2_global_slat_path):
                                trellis2_slat = np.load(trellis2_global_slat_path)
                                trellis2_slat_feats = torch.from_numpy(trellis2_slat['feats']).float()
                                trellis2_slat_coords = torch.from_numpy(trellis2_slat['coords']).int()
                                if self.config.trellis2_slat_norm and self.trellis2_slat_normalization is not None:
                                    std = torch.tensor(self.trellis2_slat_normalization['std'])[None].to(trellis2_slat_feats.device)
                                    mean = torch.tensor(self.trellis2_slat_normalization['mean'])[None].to(trellis2_slat_feats.device)
                                    trellis2_slat_feats = (trellis2_slat_feats - mean) / std
                                trellis2_slat_feats_list.append(trellis2_slat_feats)
                                trellis2_slat_coords_list.append(trellis2_slat_coords)
                ...
                for i in range(num_parts):
                    ...
                    if self.stage == '2':
                        ...
                        slat_feats_list.append(slat_feats)
                        slat_coords_list.append(slat_coords)
                        if self.trellis2_part_slat_dir is not None:
                            trellis2_slat_path = os.path.join(self.trellis2_part_slat_dir, part_id, f'{i}.npz')
                            if not os.path.exists(trellis2_slat_path) and self.id_mapping:
                                trellis2_slat_path = os.path.join(self.trellis2_part_slat_dir, self.id_to_sha256[part_id], f'{i}.npz')
                            if os.path.exists(trellis2_slat_path):
                                trellis2_slat = np.load(trellis2_slat_path)
                                trellis2_slat_feats = torch.from_numpy(trellis2_slat['feats']).float()
                                trellis2_slat_coords = torch.from_numpy(trellis2_slat['coords']).int()
                                if self.config.trellis2_slat_norm and self.trellis2_slat_normalization is not None:
                                    std = torch.tensor(self.trellis2_slat_normalization['std'])[None].to(trellis2_slat_feats.device)
                ...
                if self.stage == '2':
                    ...
                    if len(trellis2_slat_feats_list) > 0:
                        samples_trellis2_slat_feats.append(trellis2_slat_feats_list)
                        samples_trellis2_slat_coords.append(trellis2_slat_coords_list)
        ...
        if self.stage == '2':
            ...
            if len(samples_trellis2_slat_feats) > 0:
                return_dict["samples_trellis2_slat_feats"] = samples_trellis2_slat_feats
                return_dict["samples_trellis2_slat_coords"] = samples_trellis2_slat_coords
        return return_dict
    ...
    def collate_fn(self, examples):
        ...
        if "samples_trellis2_slat_feats" in examples[0].keys():
            batch_trellis2_slat_feats = [example["samples_trellis2_slat_feats"] for example in examples]
            batch_trellis2_slat_coords = [example["samples_trellis2_slat_coords"] for example in examples]
            return_dict["batch_trellis2_slat_feats"] = batch_trellis2_slat_feats
            return_dict["batch_trellis2_slat_coords"] = batch_trellis2_slat_coords
