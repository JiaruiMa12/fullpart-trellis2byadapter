...
def check_tensor(tensor, shape, throw=True):
    """Local implementation of check_tensor to avoid kaolin dependency."""
    ...
    if len(shape) != tensor.dim():
        if throw:
            raise ValueError(f"Expected {len(shape)} dimensions, got {tensor.dim()}")
        return False
    
    for i, s in enumerate(shape):
        if s is not None and s != tensor.shape[i]:
            if throw:
