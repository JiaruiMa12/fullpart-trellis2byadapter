...
def sparse_windowed_scaled_dot_product_self_attention(
    ...
    if all([seq_len == window_size for seq_len in seq_lens]):
        ...
        if ATTN == 'xformers':
            ...
            qkv_split = qkv_feats.unbind(dim=1)
            outs = []
            start = 0
            for seq_len in seq_lens:
                q_i, k_i, v_i = [
                    x[start:start + seq_len].permute(1, 0, 2).unsqueeze(0)
                    for x in qkv_split
                ]
                outs.append(F.scaled_dot_product_attention(q_i, k_i, v_i).squeeze(0).permute(1, 0, 2))
                start += seq_len
