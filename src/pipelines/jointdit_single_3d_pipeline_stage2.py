...
class JointDiTSingle3DPipelineStage2(T2VPixArtAlphaPipeline, PipelineMixin):
    ...
    def __init__(
        ...
        if os.environ.get("FULLPART_STAGE2_FP32", "0") == "1":
    ...
    def __call__(
        ...
        print("====np====", num_parts)
        img_conds = self.voxel_vae.encode_cond_img_from_tensor(img_conds)
        img_conds = img_conds.to(self.transformer.dtype)
        ...
        # Save Global SLat (all parts concatenated) for adapter conversion
        if len(voxel_latents.feats.shape) == 2:  # [N, 8]
            global_slat_feats = voxel_latents.feats.float().cpu().numpy()
            global_slat_coords = voxel_latents.coords.cpu().numpy()
            np.savez(os.path.join(save_path, 'global_slat.npz'), 
                     feats=global_slat_feats, 
                     coords=global_slat_coords)
            print(f"=== Saved Global SLat to {os.path.join(save_path, 'global_slat.npz')} ===")
            
            # Save individual part SLats for direct part-level adapter conversion
            # This avoids needing to split global_slat later
            for i in range(num_parts):
                part_mask = global_slat_coords[:, 0] == i
                part_feats = global_slat_feats[part_mask]
                part_coords = global_slat_coords[part_mask]
                # Remove batch_id column (first column) for part-level processing
                part_coords = part_coords[:, 1:]
                np.savez(os.path.join(save_path, f'part_{i}_slat.npz'),
                         feats=part_feats,
                         coords=part_coords)
                print(f"=== Saved Part {i} SLat to {os.path.join(save_path, f'part_{i}_slat.npz')} ===")
