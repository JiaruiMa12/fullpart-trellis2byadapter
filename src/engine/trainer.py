...
class Manager(torch.nn.Module):
    def __init__(self, pipeline):
        ...
        # Handle additional modules that may not be in signature keys (e.g., voxel_vae)
        additional_modules = ['voxel_vae', 'slat_adapter']
        for module_name in additional_modules:
            if hasattr(pipeline, module_name):
                module = getattr(pipeline, module_name, None)
                if module is not None and not hasattr(self, module_name):
                    if isinstance(module, torch.nn.Module) or isinstance(module, torch.nn.Parameter):
                        print(f"[Manager] Adding additional module: {module_name}")
                        setattr(self, module_name, module)
    ...
    def save_ckpts(self, ds_state_dict, path):
        ...
        if hasattr(self, "slat_adapter"):
            print("======manage save slat adapter=====")
            save_model_from_ds("slat_adapter", ds_state_dict, path)
...
class Trainer:
    ...
    def init_state_and_logger(self):
        ...
        # Skip distributed init when not launched by torchrun/deepspeed.
        if int(os.environ.get("WORLD_SIZE", "1")) <= 1:
            print("[INFO] Single process detected, using single process mode")
            os.environ["MASTER_ADDR"] = "localhost"
            os.environ["MASTER_PORT"] = "29500"
            os.environ["LOCAL_RANK"] = "0"  # Required for PartialState
            os.environ["RANK"] = "0"
