...
class Args:
    ...
    stage2_fp32: bool = False
    decode_part_ids: Tuple[int, ...] = ()
...
def _prepare_raw_batch(image_path: Path, box_path: Path, sample_id: Optional[str], raw_path: Optional[Path], view: str = "000.png", anno_dir: Optional[Path] = None) -> Dict[str, List[torch.Tensor]]:
    if raw_path is not None:
        raw_path = Path(raw_path)
        image_path = raw_path / view if image_path is None else image_path
        # Use raw_path.name as batch_id for dataset/partversexl format
        batch_id = sample_id or raw_path.name
        if box_path is None:
            if anno_dir is not None:
                # Try JSON bbox from anno_dir first, then .npy
                json_path = anno_dir / batch_id / f"{batch_id}_info.json"
                npy_path = anno_dir / batch_id / f"{batch_id}.npy"
                if json_path.exists():
                    box_path = json_path
                elif npy_path.exists():
                    box_path = npy_path
                else:
                    raise FileNotFoundError(f"No bbox file found at {json_path} or {npy_path}")
            else:
                # Use .npy bbox (demo format)
                box_path = raw_path.parent / (raw_path.name + ".npy")
    else:
...
def main(args: Args) -> None:
    ...
    if not args.skip_stage2:
        stage2_pipeline = _instantiate_stage2(
            args.stage2, stage1_output_dir, stage2_output_dir, device, fp32=args.stage2_fp32
        )
    ...
    if args.raw_path is not None or (args.raw_image is not None and args.raw_box is not None):
        all_batches.append(_to_device_dtype(
            _prepare_raw_batch(args.raw_image, args.raw_box, args.raw_sample_id, args.raw_path, args.view, args.anno_dir), device, 'bf16'))
    ...
    for batch in all_batches:
        ...
        decode_part_id = list(args.decode_part_ids) if len(args.decode_part_ids) > 0 else []
        _stage2_inference(stage2_pipeline, batch, stage2_output_dir, decode_part_id=decode_part_id)
